...
def test_predict_score():
    ...
    response = client.get("/predict_score", params={"team1": "Chelsea", "team2": "Arsenal"})
...
def test_predict_score_wrong_team():
    ...
    response = client.get("/predict_score", params={"team1": "Chelsea", "team2": "No Team"})

    assert response.status_code == 422

def test_simulate_match_from_score():

    response = client.get("/simulate_match_from_score", params={"score1":1.0, "score2":1.5})
...
def test_simulate_match():
    ...
    response = client.get("/simulate_match", params={"team1": "Chelsea", "team2": "Arsenal"})

    assert response.status_code == 200

    for key in ["team1", "team2", "proj_score1", "proj_score2", "prob1", "prob2", "probtie", "cs1", "cs2"]:
